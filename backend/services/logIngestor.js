...
const { getOpenSearchClient, getIndexNameForDate, createIndexTemplates, deleteOldIndices } = require('../config/opensearch');
  ...
  try {
    ...
    const runAtMidnight = async () => {
      try {
        ...
        console.error('Error during nightly index maintenance:', error);
        // Don't rethrow - we want to keep running
      }

      // Schedule the next run (outside try-catch so it always runs)
      const now = new Date();
